...
import { BtnNavVendor, Spinner, TableV2 } from '../../../components'
...
export const DeletedVendorsView = () => {
  ...
  const filteredVendor: VendorDeletedType = useMemo(() => {
    return {
      ...
      transceivers:
        queryVendorsDeleted.data?.transceivers.filter(transceiver =>
          transceiver.vendor.vendorName?.toLocaleLowerCase().includes(filter.toLocaleLowerCase()),
        ) || [],
      boards:
        queryVendorsDeleted.data?.boards.filter(board => board.vendor.vendorName?.toLocaleLowerCase().includes(filter.toLocaleLowerCase())) || [],
    }
    ...
    <main className='flex-1 bg-gray-300 font-oswald'>
      <div className='flex items-start gap-6 px-4 my-6'>
        <h2 className='basis-1/2 text-4xl uppercase font-bold text-right'>
          Proveedores / Vendors <span className='text-red-500'>Eliminados</span>
        </h2>
        <h3 className='basis-1/2 text-lg uppercase font-bold text-left'>
          Verifica y Limpia los Registros de Proveedores que aun se encuentran asociados a distintos equipamientos
        </h3>
      </div>
        ...
        queryVendorsDeleted.data && (
          <>
            <div className='font-roboto text-sm flex items-center justify-center gap-4 bg-emerald-300 w-fit mx-auto px-10 py-2 rounded-md'>
              <p className='uppercase bg-emerald-50 px-3 py-2 rounded-md'>
                Cantidad de Vendor Eliminados: {queryVendorsDeleted.data?.vendors.length}
              </p>
            ...
            <div className='border border-gray-600 mx-3 mt-4 text-center rounded-lg bg-gray-50 px-3 py-2 '>
              ...
              <TableV2 data={filteredVendor.vendors} info='catalogVendor' basePath='catalog/vendor' fnDelete={handlePermanentlyDeleteVendor} />
            </div>
            <div className='border border-gray-600 mx-3 mt-4 text-center rounded-lg bg-gray-50 px-3 py-2 '>
              ...
              <TableV2 data={filteredVendor.transceivers} info='catalogTransceiver' basePath='catalog/transceiver' />
            </div>
            <div className='border border-gray-600 mx-3 mt-4 text-center rounded-lg bg-gray-50 px-3 py-2 '>
              <p className='uppercase text-xl font-bold font-roboto underline decoration-red-500 underline-offset-4'>Placas Eliminadas</p>
              <TableV2 data={filteredVendor.boards} info='catalogBoard' basePath='catalog/board' />
            </div>
          </>
        )
      )}
