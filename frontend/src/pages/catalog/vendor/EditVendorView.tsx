  ...
  return (
    <main className='flex-1 bg-gray-50 px-2 py-4 text-black font-roboto'>
      <h2 className='uppercase text-2xl font-extrabold text-center'>
        <span className='text-red-500'>Modifica / Actualiza</span> los datos del Proveedor:
      </h2>
      <h3 className='text-center py-3 text-lg font-bold'>
        <strong className='text-blue-800 uppercase'>Vendor: </strong>
        <span>{queryVendor.data?.vendorName}</span> <strong className='text-blue-800 uppercase'>Origen: </strong>
        <span>{queryVendor.data?.country}</span> <strong className='text-blue-800 uppercase'>Observaciones: </strong>
        <span>{queryVendor.data?.observation}</span>
      </h3>
      {queryParams.get('isDeleted') === 'true' && (
        <div className='bg-red-400 shadow-xl shadow-red-900 px-4 py-2 mx-auto my-6 text-center uppercase font-semibold w-3/5 rounded-lg'>
