    ...
    <div className='flex space-x-2 bg-gray-200 p-4 rounded-lg'>
      <div className='border border-gray-400 px-3 py-2 rounded-lg space-y-2 h-fit'>
        <div className='flex items-center gap-2'>
          ...
          <input type='text' id='boardName' className='p-1 w-3/5' {...register('boardName', { setValueAs: value => value.trim() })} />
        </div>
        <div className='flex items-center gap-2'>
          <label htmlFor='partNumberBoard' className='w-2/5'>
            No. Parte:
          </label>
          <input type='text' id='partNumberBoard' className='p-1 w-3/5' {...register('partNumber', { setValueAs: value => value.trim() })} />
        </div>
        <div className='flex items-center gap-2'>
        ...
        <fieldset className='h-44 overflow-y-auto border border-gray-300 p-2 outline-none rounded shadow-md bg-white'>
          {Object.values(BitsRatesEnum).map(bitRate => (
            <div key={bitRate} className='flex items-center space-x-2 px-1'>
              <input type='checkbox' id={`${bitRate}board`} value={bitRate} {...register('bitsRates')} />
      ...
      <div className='border border-gray-400 px-3 py-2 rounded-lg space-y-2 h-fit'>
        <div className='flex items-center gap-2'>
          ...
          <input type='text' id='descriptionBoard' className='p-1 w-2/3' {...register('description', { setValueAs: value => value.trim() })} />
        </div>
        <div className='flex items-center gap-2'>
          <label htmlFor='observationsBoard' className='w-1/3'>
            Observaciones:
          </label>
          <input type='text' id='observationsBoard' className='p-1 w-2/3' {...register('observations', { setValueAs: value => value.trim() })} />
        </div>
        <div className='flex items-center gap-2'>
          <label htmlFor='technologyBoard' className='w-1/3'>
            Tecnologia:
          </label>
          <select id='technologyBoard' className='p-1 w-2/3' {...register('technology')}>
            <option value=''></option>
        ...
        <div className='flex items-center gap-2'>
          ...
          <select id='roadmapBoard' className='p-1 w-2/3' {...register('roadmap')}>
