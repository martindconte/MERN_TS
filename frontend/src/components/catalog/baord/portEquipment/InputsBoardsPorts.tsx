  ...
  return (
    <div className='flex gap-3'>
      <div className='flex flex-col justify-between p-2 border border-gray-400 rounded-md'>
        ...
        <div className='flex justify-between my-1 items-center gap-1'>
          <label className='w-1/3 text-right' htmlFor='model'>
            Model
          </label>
          <input
            className='w-2/3 border border-gray-300 p-1 outline-none rounded shadow-md'
      ...
      <div className='flex justify-between items-center gap-1 border border-gray-400 px-2 rounded-md'>
        <label className='text-right' htmlFor='bitsRates'>
          Bit Rates
        </label>
        <div className='w-2/3 h-44 overflow-y-auto border border-gray-300 p-2 outline-none rounded shadow-md bg-white my-1'>
          {Object.values(BitsRatesEnum).map(bitRate => (
            <div key={bitRate} className='flex items-center space-x-2'>
              <input type='checkbox' id={bitRate} name='bitsRates' value={bitRate} onChange={event => handleOnChange(event)} />
      ...
      <div className='flex flex-col justify-between p-2 border border-gray-400 rounded-md'>
        ...
        <div className='flex justify-between items-center gap-1 bg-red-600 rounded-md'>
          <label className='w-1/3 text-right text-white' htmlFor='roadmap'>
            ELIMINADOS
          </label>
          <select
            className='w-2/3 border border-gray-300 p-1 outline-none rounded shadow-md'
      ...
      <div className='flex flex-col justify-between items-center gap-2'>
        <div className='flex gap-3 basis-1/2'>
          <button
            ...
            onClick={onSubmit}
          >
            BUSCAR<span className='material-symbols-outlined'>search</span>
          </button>
          <button
            ...
            type='button'
          >
            LIMPIAR <span className='material-symbols-outlined'>backspace</span>
          </button>
        </div>
        {selectedData && setSelectedData && (
          <div className='flex flex-col justify-center items-center gap-2 basis-1/2'>
            <p className='font-roboto_condensed text-sm font-bold uppercase'>
              Cant Seleccionada: <span className='font-normal'>{selectedData?.length}</span>
            </p>
            <button
            ...
            >
              Limpiar TODA Seleecion<span className='material-symbols-outlined'>mop</span>
            </button>
          </div>
        )}
      </div>
