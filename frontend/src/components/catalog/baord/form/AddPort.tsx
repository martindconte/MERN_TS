...
export const AddPort = ({ field, index, requiredFields }: Props) => {
  ...
  return (
    <div key={field.id} className='flex flex-col items-center gap-1 px-1 py-1 w-fit'>
      ...
      {modalEquipmentPort && <AddPortEquipment close={setModalEquipmentPort} index={index} />}

      {modalLogicalFacilities && <AddPortFacilitiesModal close={setModalLogicalFacilities} index={index} />}
      ...
      <div className='flex flex-wrap px-2 py-1 rounded-md bg-stone-200'>
        ...
        <div className='flex gap-2 items-center px-2 py-1 rounded-md'>
          ...
          <select id={`${field.id}-${index}-${field.type}`} className='px-1 py-1 outline-none' {...register(`ports.${index}.type`)}>
            <option value=''></option>
