...
export class SearchBoardDTO {
  ...
  private static validKeys = new Set<keyof IBoard>(Object.values(BoardEntity.getPublicKeys) as Array<keyof IBoard>);
  ...
  static searchBoard(queryParams: Record<string, any>): IBoardSearch {
    ...
    for (const [key, value] of Object.entries(rest)) {
      if (this.validKeys.has(key as keyof BoardEntity)) {
        const processedValue = this.processValue(key as keyof BoardEntity, value);
        if (processedValue) searchParams[key as keyof BoardEntity] = this.processValue(key as keyof BoardEntity, value);
